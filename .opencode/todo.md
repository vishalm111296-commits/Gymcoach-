...
## Phase 7 — Adaptive Programming | status: in_progress

### M7.1: AdaptiveProgramEngine pure-JVM core | agent:Worker
...
### M7.2: AdaptiveProgramEngineTest (pure JVM) | agent:Worker | depends:M7.1
...
### M7.3: Assessment + engine wiring (non-forbidden files) | agent:Worker | depends:M7.2
- [ ] S7.3.1: ProgressViewModel.kt — inject AdaptiveProgramEngine; compute adapt(assessment, trainingBalance, stallWeeks) in load() where assessment+trainingBalance already exist; add adaptiveActions: List<AdaptiveProgramAction> to ProgressUiState; stallWeeks derived from VtaperBalance staleness of progress? use 0 (no historical stall data wired yet — documented, Phase 8 wires real stall detection) with code comment; add field | size:S
- [ ] S7.3.2: Create presentation/progress/components/AdaptivePlanCard.kt — ASCII card (DarkSurface, tokens) titled "ADAPTIVE PLAN"; lists adaptiveActions as "TYPE: title — detail", node count cap in UI not engine; renders empty-state "Maintain current program" only if engine returned none (engine guarantees non-empty) | size:S
- [ ] S7.3.3: ProgressDashboardScreen.kt — render AdaptivePlanCard under VShapeAssessmentCard (after vShapeAssessment block); no forbidden files | size:S

### M7.4: CI Gate | agent:Worker | depends:M7.3
- [ ] S7.4.1: Stage ONLY explicit intended paths (engine, engine test, AdaptivePlanCard, ProgressViewModel.kt, ProgressDashboardScreen.kt, todo.md, work-log.md, status.md, docs). Verify ZERO forbidden files staged (7 UI files + WorkoutSessionScreen). Commit to phase5-recovery-verified, push, dispatch gh workflow run android-build.yml --ref phase5-recovery-verified, capture run ID + totals | size:M

### M7.5: Verification Gate | agent:Reviewer | depends:M7.4
- [ ] S7.5.1: Verify commit-scope diff (git diff HEAD~1 HEAD --name-only) zero forbidden + no test weakening (only ADDITIONS), CI evidence (run ID + totals; expected 281 prior + ~12 new = ~293), offline COMPILE_EXIT=0 + JUnit OK; mark Phase 7 [x] | size:M
