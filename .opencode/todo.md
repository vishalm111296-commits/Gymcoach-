...
## Phase 6 — V-Shape Assessment | status: completed
...
### M6.3: Progress tab wiring (committed files only — none forbidden) | agent:Worker | depends:M6.2 | status: completed
...
## Phase 7 — Adaptive Programming | status: completed

### M7.1: AdaptiveProgramEngine pure-JVM core | agent:Worker | status:completed
- [x] S7.1.1: Create app/src/main/kotlin/com/gymcoach/app/core/program/AdaptiveProgramEngine.kt — pure-JVM object with data class AdaptiveProgramAction(type: AdaptiveActionType, title: String, detail: String, muscleName: String?) + enum AdaptiveActionType { VOLUME_SHIFT, DELOAD, LOAD_BUMP, VARIATION, BALANCED }. fun adapt(assessment: VShapeAssessment, balance: TrainingBalance, stallWeeks: Int): List<AdaptiveProgramAction>. Rules (product heuristics, ASCII, Locale.US, conservative language, NOT clinical): (1) VOLUME_SHIFT for each V-taper muscle (Lats, Lateral Deltoid primary; Rear Deltoid, Upper Chest, Upper Back secondary) with weeklyVolume < 10 (INSUFFICIENT) -> "Add <muscle> volume", "Weekly <muscle> volume (x.x) is below the evidence band (10-18 sets/week). Add 2-4 sets spread across sessions."; (2) DELOAD for each V-taper muscle with weeklyVolume > 21 (EXCESSIVE) -> "Reduce <muscle> volume", "Weekly <muscle> volume (x.x) is above the evidence band. Reduce 2-4 sets/week or take a light week."; (3) if neither shift nor deload emitted for the 5 muscles: LOAD_BUMP when VtaperBalance-derived primaryScore >= 3.0 and secondaryScore >= 3.0 (compute from balance via same formula as VolumeCalculator.calculateVtaperBalance: primary=(lat+lateralDelt ordinals)/2, secondary=(rearDelt+upperChest+upperBack ordinals)/3) -> "Progress load on balanced muscles", "All V-taper muscles are in or near the evidence band; increase load when all sets hit the top of the rep range."; (4) VARIATION when stallWeeks >= 3 -> "Try a variation after a stall", "No progression for 3+ weeks; swap one stalled exercise for a similar movement to reintroduce a stimulus."; (5) if empty -> single BALANCED -> "Maintain current program", "All V-taper muscles are in or near the evidence band (10-18 sets/week).". Ordering deterministic: VOLUME_SHIFT (muscle asc) -> DELOAD (muscle asc) -> VARIATION -> LOAD_BUMP -> BALANCED. Assessment.level NOT_ENOUGH_DATA must NOT suppress program actions. | size:M

### M7.2: AdaptiveProgramEngineTest (pure JVM) | agent:Worker | depends:M7.1 | status:completed
- [x] S7.2.1: Create app/src/test/kotlin/com/gymcoach/app/core/program/AdaptiveProgramEngineTest.kt — JUnit4 ASCII test names: all-muscles-insufficient -> 5 VOLUME_SHIFT; single-muscle insufficient -> only that muscle; rear-delt excessive -> DELOAD only; lat excessive -> DELOAD only; optimal scores + in-band volumes -> LOAD_BUMP; stallWeeks=3 -> VARIATION appended/positioned; perfect program (late-optimal, in-band, stall<3) -> single BALANCED; NOT_ENOUGH_DATA assessment still yields program actions; deterministic ordering (VOLUME_SHIFT before DELOAD/VARIATION/LOAD_BUMP); no NaN/negative volume inputs crash; Locale.US formatting of volume (x.x one decimal) | size:M

### M7.3: Assessment + engine wiring (non-forbidden files) | agent:Worker | depends:M7.2 | status:completed
- [x] S7.3.1: ProgressViewModel.kt — inject AdaptiveProgramEngine; compute adapt(assessment, trainingBalance, stallWeeks) in load() where assessment+trainingBalance already exist; add adaptiveActions: List<AdaptiveProgramAction> to ProgressUiState; stallWeeks derived from VtaperBalance staleness of progress? use 0 (no historical stall data wired yet — documented, Phase 8 wires real stall detection) with code comment; add field | size:S
- [x] S7.3.2: Create presentation/progress/components/AdaptivePlanCard.kt — ASCII card (DarkSurface, tokens) titled "ADAPTIVE PLAN"; lists adaptiveActions as "TYPE: title — detail", node count cap in UI not engine; renders empty-state "Maintain current program" only if engine returned none (engine guarantees non-empty) | size:S
- [x] S7.3.3: ProgressDashboardScreen.kt — render AdaptivePlanCard under VShapeAssessmentCard (after vShapeAssessment block); no forbidden files | size:S

### M7.4: CI Gate | agent:Worker | depends:M7.3 | status:completed
- [x] S7.4.1: Stage ONLY explicit intended paths (engine, engine test, AdaptivePlanCard, ProgressViewModel.kt, ProgressDashboardScreen.kt, todo.md, work-log.md, status.md, docs). Verify ZERO forbidden files staged (7 UI files + WorkoutSessionScreen). Commit to phase5-recovery-verified, push, dispatch gh workflow run android-build.yml --ref phase5-recovery-verified, capture run ID + totals | size:M

### M7.5: Verification Gate | agent:Reviewer | depends:M7.4 | status:completed
- [x] S7.5.1: Verify commit-scope diff (git diff HEAD~1 HEAD --name-only) zero forbidden + no test weakening (only ADDITIONS), CI evidence (run ID + totals; expected 281 prior + ~12 new = ~293), offline COMPILE_EXIT=0 + JUnit OK; mark Phase 7 [x] | size:M
